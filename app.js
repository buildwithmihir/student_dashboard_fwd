...
window.onload = () => {
  if (load('theme') === 'dark') {
    ...
    el('theme-icon').textContent = '☀️';
  }
  renderTodos(); renderGPA(); renderAttendance(); renderNotes();
...
function renderTodos() {
  ...
  el('todo-list').innerHTML = tasks.map((t, i) => `
    ...
    </li>`).join('') || '<p style="color:var(--muted)">No tasks.</p>';
  el('task-count').textContent = tasks.filter(t => !t.completed).length + ' remaining';
}

function addTodo() {
  const inp = el('todo-input'), dat = el('todo-date');
  if (!inp.value.trim()) return;
  const tasks = load('todos');
  tasks.push({ text: inp.value.trim(), completed: false, date: dat.value });
  save('todos', tasks);
  inp.value = dat.value = '';
  renderTodos();
}
...
function clearAll() {
  if (confirm('Delete all tasks?')) { save('todos', []); renderTodos(); }
}
    ...
    el('gpa-subjects').innerHTML = SUBJECTS.map(s => `
      <div class="gpa-row">
        ...
        <input id="gpa-${s.n}" type="number" min="0" max="100" placeholder="Marks"
          value="${marks[s.n] ?? ''}" oninput="liveGP('${s.n}', this.value)">
        <span id="gp-${s.n}">GP: <b>${marks[s.n] != null ? gp(marks[s.n]) : '—'}</b></span>
      </div>`).join('');
  calcGPA(marks);
}

function liveGP(name, raw) {
  const marks = load('gpa-marks');
  if (raw === '') { delete marks[name]; }
  else {
    const n = Math.min(100, Math.max(0, +raw));
    marks[name] = n;
    el('gp-' + name).innerHTML = 'GP: <b>' + gp(n) + '</b>';
  }
  save('gpa-marks', marks); calcGPA(marks);
}

function calcGPA(marks) {
  let pts = 0, cr = 0;
  SUBJECTS.forEach(s => { if (marks[s.n] != null) { pts += gp(marks[s.n]) * s.c; cr += s.c; } });
  el('gpa-display').textContent = cr ? (pts / cr).toFixed(2) : '0.00';
}

function addSubject() {
  const inp = el('sub-input');
  if (!inp.value.trim()) return;
  const att = load('attendance');
  if (!att[inp.value.trim()]) att[inp.value.trim()] = { present: 0, total: 0 };
  save('attendance', att); inp.value = ''; renderAttendance();
}

function markPresent(i) { mark(i, true); }
function markAbsent(i)  { mark(i, false); }

function mark(i, present) {
  const att = load('attendance'), key = Object.keys(att)[i];
  if (!key) return;
  if (present) att[key].present++;
  att[key].total++;
  save('attendance', att); renderAttendance();
}
...
function renderAttendance() {
  ...
  let op = 0, ot = 0;
  el('att-list').innerHTML = keys.map((sub, i) => {
    const d = att[sub];
    ...
    return `<div class="card">
      ...
      <div class="att-actions">
        <button onclick="markPresent(${i})">✓</button>
        <button onclick="markAbsent(${i})">✗</button>
        <button class="del" onclick="deleteSub(${i})">✕</button>
      </div>
    </div>`;
  }).join('') || '<p style="color:var(--muted)">No subjects.</p>';
  el('overall-att').textContent = (ot ? ((op / ot) * 100).toFixed(1) : 0) + '%';
}
